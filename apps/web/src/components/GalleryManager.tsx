...
export function GalleryManager({ event }: { event: EventItem }) {
  ...
  async function replaceGalleryPhotos(galleryId: string) {
    if (!selectedCount) { setMessage('Əvvəl gallery üçün yeni foto seçimi edin.'); return }
    if (allMatching) { setMessage('Gallery yenilənməsi üçün konkret fotoları seçin; “hamısını seç” bu əməliyyatda istifadə edilmir.'); return }
    setBusy(true); setMessage('')
    try { await request(`/api/host/events/${event.id}/galleries/${galleryId}/photos`, { method: 'PUT', body: JSON.stringify({ photoIds: [...selectedIds] }) }); clearSelection(); setEditingGalleryId(null); await load(page?.page ?? 1); setMessage('Gallery fotoları yeniləndi.') }
    catch (error) { setMessage(error instanceof Error ? error.message : 'Gallery fotoları yenilənmədi.') } finally { setBusy(false) }
  }
  async function updateGalleryPin(galleryId: string) {
    const pin = window.prompt('Yeni PIN-i daxil edin (4–12 rəqəm):')
    if (!pin) return
    setBusy(true); setMessage('')
    try { await request(`/api/host/events/${event.id}/galleries/${galleryId}/pin`, { method: 'PATCH', body: JSON.stringify({ pin }) }); await load(page?.page ?? 1); setMessage('Gallery PIN-i yeniləndi.') }
    catch (error) { setMessage(error instanceof Error ? error.message : 'PIN yenilənmədi.') } finally { setBusy(false) }
  }
  async function deleteGallery(gallery: SharedGallery) {
    if (!window.confirm(`“${gallery.name}” gallery-si silinsin? Public link artıq açılmayacaq.`)) return
    setBusy(true); setMessage('')
    try { await request(`/api/host/events/${event.id}/galleries/${gallery.id}`, { method: 'DELETE' }); await load(page?.page ?? 1); setMessage('Gallery silindi.') }
    catch (error) { setMessage(error instanceof Error ? error.message : 'Gallery silinmədi.') } finally { setBusy(false) }
  }
  const totalPages = page ? Math.max(1, Math.ceil(page.totalCount / page.pageSize)) : 1
  return <section className="panel gallery-panel"><div className="panel-heading"><div><p className="eyebrow">Foto qalereyası</p><h2>{page?.totalCount ?? 0} foto</h2><p className="muted">{selectedCount ? `${selectedCount} seçilib` : editingGalleryId ? 'Yeni foto seçin, sonra gallery-ni yeniləyin.' : 'Seçim edin və public gallery yaradın.'}</p></div><div className="gallery-tools"><button className="text-button" type="button" disabled={busy} onClick={() => void queueExport()}>ZIP hazırla</button><label className="gallery-filter">QR filtri<select value={filter} onChange={item => setFilter(item.target.value)}><option value="">Bütün QR-lər</option>{invitations.map(item => <option key={item.id} value={item.id}>{item.label ?? 'Adsız QR'}</option>)}</select></label></div></div><div className="bulk-actions"><button className="text-button" type="button" onClick={togglePage}>{currentPageSelected ? 'Səhifə seçimini ləğv et' : 'Səhifəni seç'}</button><button className="text-button" type="button" onClick={() => { setAllMatching(value => !value); setSelectedIds(new Set()) }}>{allMatching ? 'Bütün nəticələr seçimini ləğv et' : 'Bu filtr üzrə hamısını seç'}</button>{selectedCount ? <><button className="text-button" type="button" onClick={clearSelection}>Seçimi təmizlə</button>{editingGalleryId ? <button className="primary" type="button" disabled={busy} onClick={() => void replaceGalleryPhotos(editingGalleryId)}>Bu seçimlə gallery-ni yenilə</button> : <button className="danger-button" type="button" disabled={busy} onClick={() => void removeSelected}>Seçilənləri sil</button>}</> : null}{editingGalleryId ? <button className="text-button" type="button" onClick={() => setEditingGalleryId(null)}>Gallery redaktəsini bağla</button> : null}</div><form className="share-form" onSubmit={formEvent => { formEvent.preventDefault(); void createGallery(formEvent.currentTarget) }}><label>Public gallery adı<input name="name" maxLength={120} required placeholder="Məsələn: Ailə fotoları" /></label><label>PIN (4–12 rəqəm)<input name="pin" inputMode="numeric" pattern="[0-9]{4,12}" minLength={4} maxLength={12} required /></label><button className="primary" disabled={busy || !selectedCount}>Seçilənlərdən link yarat</button></form>{message ? <p className="error" role="alert">{message}</p> : null}<div className="photo-grid">{page?.items.map(photo => <article className="photo-card" key={photo.id}><label className="photo-select"><input type="checkbox" checked={allMatching || selectedIds.has(photo.id)} onChange={() => togglePhoto(photo.id)} disabled={allMatching} /><span>Seç</span></label><div className="photo-preview">{photo.thumbnailUrl ? <img src={photo.thumbnailUrl} alt={`${photo.originalFileName} önizləmə`} /> : <span>Önizləmə hazırlanır...</span>}</div><div><strong title={photo.originalFileName}>{photo.originalFileName}</strong><span>{photo.invitationLabel ?? 'Adsız QR'}</span></div></article>)}</div>{page && page.totalCount > page.pageSize ? <div className="pagination"><button className="text-button" disabled={page.page <= 1} onClick={() => void load(page.page - 1)}>← Əvvəlki</button><span>{page.page} / {totalPages}</span><button className="text-button" disabled={page.page >= totalPages} onClick={() => void load(page.page + 1)}>Növbəti →</button></div> : null}<div className="share-links">{galleries.map(gallery => <article className="share-link" key={gallery.id}><strong>{gallery.name}</strong><span>{gallery.photoCount} foto</span><code>{`${window.location.origin}/g/${gallery.publicId}`}</code><div className="inline-actions"><button className="text-button" type="button" onClick={() => void copy(gallery.publicId)}>Linki kopyala</button><button className="text-button" type="button" disabled={busy} onClick={() => { clearSelection(); setEditingGalleryId(gallery.id) }}>Fotoları yenilə</button><button className="text-button" type="button" disabled={busy} onClick={() => void updateGalleryPin(gallery.id)}>PIN dəyiş</button><button className="danger-button" type="button" disabled={busy} onClick={() => void deleteGallery(gallery)}>Sil</button></div></article>)}</div><div className="export-list">{exports.map(job => <p className="muted" key={job.id}>ZIP: {job.status}{job.downloadUrl ? <a href={job.downloadUrl}> — endir</a> : null}{job.error ? ` — ${job.error}` : null}</p>)}</div></section>
}
