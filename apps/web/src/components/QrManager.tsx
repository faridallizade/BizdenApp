...
export function QrManager({ event }: { event: EventItem }) {
  ...
  async function regenerate(id: string) { if (!window.confirm('Köhnə QR dərhal deaktiv olacaq və yeni QR endirmək üçün açılacaq. Davam edək?')) return; setBusy(true); setMessage(''); try { const result = await request<InvitationToken>(`/api/host/events/${event.id}/invitations/${id}/regenerate`, { method: 'POST' }); setTokens([result]); await load(); setMessage('Yeni QR hazırdır. Aşağıdan PNG və ya brendli PDF kimi endirin.') } catch (error) { setMessage(error instanceof Error ? error.message : 'QR yenilənmədi.') } finally { setBusy(false) } }
  ...
  return <section className="panel qr-panel"><div className="panel-heading"><div><p className="eyebrow">Qonaq dəvətləri</p><h2>QR kodlar</h2><p className="muted">{event.name} · {invitations.length} QR</p></div></div><form className="qr-create" onSubmit={create}><label>QR etiketi<input name="label" placeholder="Məsələn: Ana masa" maxLength={120} /></label><label>Foto limiti<input name="uploadLimit" type="number" min="1" max="10000" defaultValue="15" required /></label><label>Sayı<input name="count" type="number" min="1" max="50" defaultValue="1" required /></label><button className="primary" disabled={busy}>QR yarat</button></form>{message ? <p className="notice" role="status">{message}</p> : null}{tokens.length ? <div className="token-box"><strong>Yeni QR-lər — indi endirin və ya kopyalayın.</strong><p>Raw token bazada saxlanmır. Mövcud QR-i endirmək üçün siyahıdakı “Yenilə və endir” düyməsi ilə yeni, təhlükəsiz token yaradın.</p><button className="text-button" type="button" disabled={busy} onClick={() => void downloadBatchPdf}>Bütün yeniləri bir PDF-də endir</button><div className="qr-preview-list">{tokens.map(item => <QrPreview key={item.invitation.id} item={item} event={event} />)}</div></div> : null}<div className="invitation-list">{invitations.map(item => <article className="invitation" key={item.id}><div><strong>{item.label ?? 'Adsız QR'}</strong><p>{item.completedUploads}/{item.uploadLimit} foto · {item.isActive ? 'Aktiv' : 'Deaktiv'}</p></div><div className="invitation-actions"><button className="text-button" disabled={busy} onClick={() => void toggle(item)}>{item.isActive ? 'Deaktiv et' : 'Aktiv et'}</button><button className="text-button" disabled={busy} onClick={() => void regenerate(item.id)}>Yenilə və endir</button></div></article>)}{!invitations.length ? <p className="muted empty">Hələ QR kod yoxdur.</p> : null}</div></section>
}
